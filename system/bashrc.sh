...
if [ ! -f /.dockerenv ] && ! grep -q 'docker\|lxc' /proc/1/cgroup; then
  ...
  declare -A commands=(
      ...
      ["dps"]='dps() { printf "%-12s %-15s %-30s %-60s %s\n" "NAME" "STATUS" "NETWORK" "PORTS" "IMAGE"; docker ps -a --format "{{.Names}}|{{.Status}}|{{.Image}}|{{.Ports}}" | sort | while IFS="|" read -r name state_info image_info port_info; do network_info=$(docker inspect --format "{{range \$k,\$v := .NetworkSettings.Networks}}{{printf \"%s:%s\" \$k \$v.IPAddress}}{{end}}" "$name"); printf "%-12s %-15s %-30s %-60s %s\n" "$name" "${state_info:0:12}" "$network_info" "${port_info:0:57}" "$image_info"; done; }'
