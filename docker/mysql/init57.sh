...
if [ $# -ge 1 ]; then
    ...
    read -p "容器名称 [默认：mysql57]: " container
    container=${container:-mysql57}
fi
...
# 在容器内执行配置操作
docker exec $container bash -c "cp /conf/mysql57.cnf /etc/mysql/conf.d/mysql.cnf"
